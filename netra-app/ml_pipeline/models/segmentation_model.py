...
class PrismDRYoloWrapper:
    ...
    Uses ONNX format for lightning-fast, ultra-low RAM usage (safe for 512MB limits).
    """
    def __init__(self, checkpoints_dir: str):
        ...
        for lesion_name, filename in lesions.items():
            ...
            if os.path.exists(path):
                print(f"[NetramNova Pipeline] Loading PRISM-DR {lesion_name} ONNX model...", file=sys.stderr)
                self.models[lesion_name] = YOLO(path, task='detect')
            else:
                print(f"[NetramNova Pipeline] Warning: PRISM-DR YOLO ONNX weights not found: {path}", file=sys.stderr)
    ...
    def predict_lesions(self, image_bgr) -> list[tuple[int, int, int, str, float]]:
        ...
        for lesion_name, model in self.models.items():
            res = model(image_bgr, verbose=False, imgsz=512)[0]
            boxes = res.boxes
